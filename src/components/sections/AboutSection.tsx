...
import { BrainCircuit, Layers3, LayoutGrid } from "lucide-react";
...
const featureIcons = [Layers3, BrainCircuit, LayoutGrid];
  ...
  return (
    ...
    >
      <Container className="grid gap-10 lg:grid-cols-[0.76fr_1fr] lg:gap-16">
        ...
        >
          <SectionBadge>{siteData.about.badge}</SectionBadge>
          <h2
            id="about-heading"
            ...
            {siteData.about.features.map((feature, index) => {
              ...
              return (
                <Card key={feature.title} className="rounded-3xl">
                  ...
                  <h3 className="mt-4 text-sm font-semibold text-[#1c1a17]">{feature.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[#5a544c]">{feature.body}</p>
                </Card>
              );
            })}
          </div>
