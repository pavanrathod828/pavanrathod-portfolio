...
- **Current phase:** Phase 8.1 complete. Three targeted post-Phase-8 fixes shipped: stale `resume.pdf` deleted from `public/` (Resume links now degrade gracefully via the existing build-time `existsSync` check — nav omits, contact shows "Resume — coming soon" inert span, footer no longer has a link row at all), duplicate Footer link row removed (Footer is now just the © credit line), and NowSection strip middots normalized via explicit `{" · "}` JSX expressions so STACK and CREDENTIALS lines have uniform spacing.
- **Last completed:** Phase 8.1 fix commit `cc4afa6` (−78 net lines). `public/resume.pdf` git-removed (was a 196KB outdated PDF that the build-time check was correctly resolving to "active link" — removing the file flips `hasResume` to `false` so the conditional logic delivers the inert/omitted behavior originally intended). `SiteFooter.tsx` simplified to just `<footer><p className="site-footer-credit">…</p></footer>` — `hasResume` prop dropped, `.site-footer-links` / `.site-footer-link` CSS deleted. `NowSection.tsx` strip lines rewritten with `{" · "}` JSX expression separators throughout — fixes the SWC JSX-whitespace quirk that was eating the leading space on the CREDENTIALS line.
- **Next action:** Phase 9 — pre-merge parity audit. Full desktop + mobile pass on the Vercel preview, Lighthouse / a11y audit, real-content sweep (no placeholder copy), then squash-merge to `main` only on explicit "merge it". **Pre-merge item tracked: replace the deleted `public/resume.pdf` with a rebuilt résumé before merge** so all three Resume links go fully live on production. Until that file is added back, the live site ships with `hasResume=false` and the inert Resume treatment.
